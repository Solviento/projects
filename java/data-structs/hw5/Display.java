...
public class Display extends JFrame {
			...
			while ((line = vertexFileBr.readLine()) != null) {
				...
				if (parts.length != 3) {
					...
					throw new IOException(
							"Invalid line in vertex file " + line);
				}
	...
	public Display() {
		...
		JButton btnComputeAllEuclidean = new JButton(
		...
		contentPane.add(weightedStartCityComboBox,
				gbc_weightedStartCityComboBox);
		...
		lblPanda.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) {
				try {
					Desktop.getDesktop()
							.browse(new URI("http://linanqiu.github.io/"));
		...
		btnDrawWeightedShortest.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseReleased(MouseEvent e) {
				String startCity = weightedStartCityComboBox.getItemAt(
						weightedStartCityComboBox.getSelectedIndex());
				String endCity = weightedEndCityComboBox
						.getItemAt(weightedEndCityComboBox.getSelectedIndex());
				System.out.println("Calculating shortest weighted path for "
						+ startCity + " to " + endCity);
				List<Edge> weightedPath = dijkstra.getDijkstraPath(startCity,
						endCity);
	...
	private void updateGraphPanel() {
		...
		System.out.println("Constructing new file from " + vertexFile + " and "
				+ edgeFile);
		...
		String[] cityNames = cityNameList
				.toArray(new String[cityNameList.size()]);
		weightedStartCityComboBox
				.setModel(new DefaultComboBoxModel<>(cityNames));
	...
	public class GraphPanel extends JPanel {
		...
		public void paintComponent(Graphics g) {
			...
			xFactor = (this.getBounds().width - 2 * MARGIN_X)
					/ (float) (maxX - minX);
			yFactor = (this.getBounds().height - 2 * MARGIN_Y)
					/ (float) (maxY - minY);
		...
		public void paintVertex(Graphics g, Vertex v) {
			...
			g2.drawOval(x - VERTEX_RADIUS / 2, y - VERTEX_RADIUS / 2,
					VERTEX_RADIUS, VERTEX_RADIUS);
			g2.setStroke(oldStroke);
			g2.setColor(Color.LIGHT_GRAY);
			g2.fillOval(x - VERTEX_RADIUS / 2, y - VERTEX_RADIUS / 2,
					VERTEX_RADIUS, VERTEX_RADIUS);
			g2.setColor(Color.DARK_GRAY);
			g2.drawString(v.name, x - v.name.length() * 8 / 2,
					y + VERTEX_RADIUS / 2);
		}

		public void paintEdge(Graphics g, Vertex u, Vertex v, double weight,
				Color color, int thickness, int alpha) {
			Graphics2D g2 = (Graphics2D) g;
			...
			if (chckbxShowEdgeWeights.isSelected()) {
				...
				g2.drawString(String.format("%.1f", weight), (x1 + x2) / 2,
						(y1 + y2) / 2);
				g2.setFont(oldFont);
			}
		}
